...
object Joins {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val flights_between_airports = flights.select($"origin", $"dest"). //$ is like the "col" function
      ...
      flights.registerTempTable("flights") //to perform sql with tables
      ...
      val air = airports
      val fba2 = fba.
        join(air, fba("origin") === air("iata")). //The equals are with 3 = in scala
